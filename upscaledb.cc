...
// ─────────────────────────────────────────────────────────────────────────────
// do_create_env
// Lock: acquired at entry, released before EVERY return path (3 paths).
// ─────────────────────────────────────────────────────────────────────────────
ups_status_t
...
#if defined(HFAIRLOCK)
  hfairlock_acquire(&ms_hfairlock);
#else
  ...
  if (ms_env == 0) {
    ...
    if (st) {
      LOG_ERROR(("ups_env_create failed with error %d (%s)\n",
                              st, ups_strerror(st)));
#if defined(HFAIRLOCK)
      hfairlock_release(&ms_hfairlock);  // release: early error return
#endif
      return (st);
    }
  }
...
#if defined(HFAIRLOCK)
  hfairlock_release(&ms_hfairlock);  // release: normal return
#endif
  return (st);
}
...
// ─────────────────────────────────────────────────────────────────────────────
// do_open_env
// Lock: acquired at entry, released before EVERY return path (2 paths).
// ─────────────────────────────────────────────────────────────────────────────
ups_status_t
...
UpscaleDatabase::do_close_env()
{
  ...
  if (ms_refcount == 0) {
    ...
    assert(ms_env == 0);
#if defined(HFAIRLOCK)
    hfairlock_release(&ms_hfairlock);  // release: refcount already 0
#endif
    return (0);
  }
  ...
  if (--ms_refcount > 0) {
#if defined(HFAIRLOCK)
    hfairlock_release(&ms_hfairlock);  // release: other threads still using env
#endif
    return (0);
  }
...
#if defined(HFAIRLOCK)
  hfairlock_release(&ms_hfairlock);  // release: normal return (last user)
#endif
  return (0);
}

// ─────────────────────────────────────────────────────────────────────────────
// Everything below is UNCHANGED from the original — no lock needed here
// ─────────────────────────────────────────────────────────────────────────────
